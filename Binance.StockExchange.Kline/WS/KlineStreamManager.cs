...
namespace Binance.StockExchange.Kline.WS
{
    public class KlineStreamManager
    {
        ...
        public void ConnectStreams(IEnumerable<string> pairs, IEnumerable<string> timeFrames)
        {
            if (pairs?.Count() > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < pairsList.Count; i++)
                    {
                        ...
                        {
                            var klineService = new KlineStreamService(repository, channels.Remove(channels.Length - 1, 1).ToString());
